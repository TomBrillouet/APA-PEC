...
type DataShapeType = BilanType | PatientType

type ShapeCardType = {
  dataShape: DataShapeType
  labels: LabelsType
}

export default function ShapeCard({ dataShape, labels }: ShapeCardType) {
    ...
    <ShapeCardStyled className="old-values">
      <div className="card-shape">
        ...
        <div>
          <IoIosBody />
          <span>
            {labels.imc} <b>{getImc(weight, height).toFixed(2)}</b>
          </span>
        </div>
      </div>
    </ShapeCardStyled>
